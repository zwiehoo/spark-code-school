...
object OfferPublicationStarted {
  ...
  def parse(message: String): (SellerId, Name, CountryCode, OfferId, Price) = {
    val regex =
      """.*?["]offerId["]:["](.*?)["].*?["]name["]:["](.*?)["].*?["]price["]:(.*?)[,].*?["]sellerId["]:(.*?)[,].*?["]countryIsoCode["]:["](.*?)["].*?""".r

    message.replace("currentPrice", "price") match {
      case regex(offerId, name, price, sellerId, countryCode) =>
        (SellerId(sellerId), Name(name), CountryCode(countryCode), OfferId(offerId), Price(price))
    }
  }

  case class SellerId(val sellerId: String) extends AnyVal
  case class Name(val name: String) extends AnyVal
  case class Price(val price: String) extends AnyVal
  case class CountryCode(val code: String) extends AnyVal
  case class OfferId(val id: String) extends AnyVal
}
...
object Test extends App {
  ...
  val res = OfferPublicationStarted.parse(s)
